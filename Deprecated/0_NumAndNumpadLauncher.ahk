...
; --------------------------------------------------
    ...
    if (count = 0)
    {
        if (runCommand != "")
        {
            ; 📌 [수정] 실행 직전, 현재 마우스가 위치한 모니터의 좌표를 미리 구합니다.
            coords := GetMouseMonitorCoords()

            ; 📌 [수정] 크롬 새 창이 뜨는 타이밍을 확실히 잡기 위해, 
            ; '명령어 실행 직후 가장 먼저 새로 생성되는 크롬 창(ahk_class Chrome_WidgetWin_1)'을 추적합니다.
            Run(runCommand)

            ; 크롬 창이 생성될 때까지 최대 3초 대기 (클래스명으로 대기하여 타이틀 로딩 전 미리 가로챕니다)
            if WinWait("ahk_class Chrome_WidgetWin_1", , 3)
            {
                Sleep 100 ; 크롬이 윈도우 창 구조를 완전히 생성할 시간을 아주 잠깐 제공
                
                ; 현재 가장 위에 활성화되었거나 방금 생성된 크롬 창의 HWND 획득
                hwnd := WinExist("A")

                if hwnd
                {
                    ; 📌 원래 크롬의 이전 창 크기 기억 설정을 무력화하기 위해 
                    ; 최적의 크기로 먼저 '복구(Restore)' 상태를 만든 후 이동/최대화를 진행합니다.
                    try WinRestore("ahk_id " hwnd)

                    ; 마우스가 있는 모니터 위치로 창 이동
                    try WinMove(
                        coords.X,
                        coords.Y,
                        1280, ; 기본 안전 폭
